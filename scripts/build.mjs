...
const texts = {
  ko: { work: '프로젝트', journey: '지나온 과정', about: '소개', all: '전체', data: '데이터 & 자동화', robotics: '로봇 & 자율 시스템', selected: '프로젝트 모음', explore: '프로젝트 둘러보기', timeline: '경험은 이렇게 이어졌습니다.', intro: '다음에 만들고 싶은 것.', back: '프로젝트 목록', detail: '자세히 보기', role: '나의 역할', period: '시기', stack: '사용 기술', concept: '개념도 · 실제 실행 화면이 아닙니다', media: '영상과 기록', resources: '관련 자료', next: '다음 프로젝트', skip: '본문으로 건너뛰기', count: '개 프로젝트', experience: '프로젝트 경험', simulation: '시뮬레이션', 'in-progress': '개발 중', archive: '초기 프로젝트', toc: '이 프로젝트에서', note: '이 페이지는 현재 확인된 경험을 바탕으로 작성했습니다. 성능 수치와 상세 검증 자료는 확인 후 추가합니다.', caption: '데이터와 자율 시스템에 관한 작업 기록' },
  en: { work: 'Work', journey: 'Journey', about: 'About', all: 'All projects', data: 'Data & automation', robotics: 'Robotics & autonomy', selected: 'Selected work', explore: 'Explore the projects', timeline: 'How the work connects.', intro: 'What I want to build next.', back: 'All projects', detail: 'Explore project', role: 'My contribution', period: 'Period', stack: 'Tools & technologies', concept: 'Concept diagram · not a recorded result', media: 'Videos & records', resources: 'Resources', next: 'Next project', skip: 'Skip to content', count: 'projects', experience: 'Project experience', simulation: 'Simulation', 'in-progress': 'In progress', archive: 'Early project', toc: 'In this project', note: 'This page describes the experience shared so far. Performance figures and detailed validation evidence will be added once verified.', caption: 'A working record of data and autonomous systems' }
};
...
export async function build() {
  ...
  function page(lang, slug, body, title, description) {
    ...
    return `<!doctype html><html lang="${lang}"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(title)} — ${escape(site.name)}</title><meta name="description" content="${escape(description)}"><meta name="theme-color" content="#f4f5f6"><meta property="og:type" content="website"><meta property="og:title" content="${escape(title)}"><meta property="og:description" content="${escape(description)}"><link rel="alternate" hreflang="${other}" href="${alternate}"><link rel="icon" href="${base}favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="${base}assets/style.css"><script src="${base}assets/site.js" defer></script>${slug === 'vehicle-analysis' ? `<link rel="stylesheet" href="${base}media/vehicle/case-study.css"><script src="${base}media/vehicle/scale.js" defer></script>` : ''}</head><body>
      <a class="skip" href="#main">${t.skip}</a>
      <header class="site-header"><a class="brand" href="${home}" aria-label="${escape(site.name)} home"><span class="brand-mark" aria-hidden="true">↗</span>${escape(site.name)}<span class="brand-suffix"> / PORTFOLIO</span></a><nav aria-label="${lang === 'ko' ? '주요 메뉴' : 'Main navigation'}"><a href="${home}#work">${t.work}</a><a href="${home}#journey">${t.journey}</a><a href="${home}#about">${t.about}</a></nav><div class="language" aria-label="${lang === 'ko' ? '언어' : 'Language'}">${lang === 'ko' ? '<span aria-current="true">KO</span><a href="' + alternate + '" lang="en" hreflang="en" aria-label="View this page in English">EN</a>' : '<a href="' + alternate + '" lang="ko" hreflang="ko" aria-label="이 페이지 한국어로 보기">KO</a><span aria-current="true">EN</span>'}</div></header>
      ${body}
      <footer class="site-footer"><div><a class="footer-name" href="${home}">${escape(site.name)}<span aria-hidden="true">↗</span></a><p>${t.caption}</p></div><div class="socials">${socials.map(([url, label]) => `<a href="${escape(url)}">${label} <span aria-hidden="true">↗</span></a>`).join('')}</div><p class="footer-meta">DATA / ROBOTICS / ENGINEERING</p></footer>
    </body></html>`;
  }
  for (const lang of ['ko', 'en']) {
    const t = texts[lang];
    ...
    for (const p of projects) {
      ...
      for (const item of [p.cover, ...p.media.map(m => m.src)].filter(Boolean)) {
        if (!/^media\/[a-zA-Z0-9_./-]+$/.test(item) || item.includes('..')) throw new Error(`Use a local media/ path: ${item}`);
        await access(path.join(out, item));
      }
    }
    const cards = projects.map((p, i) => `<article class="project-card" data-category="${p.category}"><a class="card-link" href="projects/${p.slug}/"><div class="project-visual ${escape(p.diagram)}"><img src="../${p.cover || `assets/${p.diagram}-${lang}.svg`}" alt="${escape(p.cover ? p.title[lang].replaceAll('\n', ' ') : `${p.title[lang].replaceAll('\n', ' ')} — ${t.concept}`)}" width="960" height="600" ${i > 1 ? 'loading="lazy"' : 'fetchpriority="high"'}><span class="visual-label">${p.cover ? t[p.category] : (lang === 'ko' ? '구조 개념도' : 'Concept diagram')}</span><span class="card-open" aria-hidden="true">↗</span></div><div class="card-meta"><span>${String(i + 1).padStart(2, '0')} / ${t[p.category]}</span><span class="stage ${p.stage === 'in-progress' ? 'active' : ''}">${t[p.stage]}</span></div><h3>${lines(p.title[lang])}</h3><p class="card-summary">${escape(p.summary[lang])}</p><div class="tags">${p.tags.map(tag => `<span>${escape(tag)}</span>`).join('')}</div></a></article>`).join('');
    const main = `<main id="main"><section class="hero"><div class="eyebrow"><span class="tiny-cross" aria-hidden="true">✳</span> DATA & AUTONOMOUS SYSTEMS</div><div class="hero-main"><h1>${lines(site.intro[lang])}</h1><div class="hero-aside"><p>${escape(site.description[lang])}</p><a class="text-link" href="#work">${t.explore} <span aria-hidden="true">↓</span></a></div></div><div class="hero-bottom"><span>PORTFOLIO / SELECTED EXPERIENCES</span><span>PYTHON · DATA · ROBOTICS</span></div></section>
      ...
      <section id="journey" class="journey-section"><div class="journey-heading"><span class="eyebrow">02 / JOURNEY</span><h2>${t.timeline}</h2><p>${lang === 'ko' ? '로봇 설계에서 데이터 처리, 그리고 업무 자동화까지.' : 'From robot design to data processing and workflow automation.'}</p></div><ol class="timeline">${site.timeline.map((entry,i) => `<li><span class="timeline-dot" aria-hidden="true"></span><p class="timeline-period">${escape(entry.period[lang])}</p><h3>${escape(entry.title[lang])}</h3><p>${escape(entry.body[lang])}</p></li>`).join('')}</ol></section>
    ...
    for (let i = 0; i < projects.length; i++) {
      ...
      const article = p.slug === 'vehicle-analysis' ? tokens.map(token => token.type === 'fence' && token.info.trim().startsWith('ubi-') ? renderVehicleBlock(token.info.trim(), lang) : md.renderer.render([token], md.options, {})).join('') : md.renderer.render(tokens, md.options, {});
      const next = projects[(i+1) % projects.length];
      const media = p.media.length ? `<section class="media-gallery"><h2>${t.media}</h2>${p.media.map(m => `<figure>${m.type === 'video' ? `<video controls playsinline preload="metadata" src="../../../${escape(m.src)}" aria-label="${escape(m.alt[lang])}"></video>` : `<img src="../../../${escape(m.src)}" alt="${escape(m.alt[lang])}" loading="lazy">`}<figcaption>${escape(m.alt[lang])}</figcaption></figure>`).join('')}</section>` : '';
      for (const link of p.links) if (!/^https:\/\//.test(link.url)) throw new Error(`Project links must use HTTPS: ${p.slug}`);
      const resources = p.links.length ? `<section class="resources"><h2>${t.resources}</h2>${p.links.map(link => `<a href="${escape(link.url)}">${escape(link.label[lang])} ↗</a>`).join('')}</section>` : '';
      ...
      const detail = `<main id="main" class="detail-main${p.slug === 'vehicle-analysis' ? ' ubi-detail' : ''}"><a class="back-link" href="../../#work">← ${t.back}</a><header class="detail-header"><div class="eyebrow">${t[p.category]} / ${t[p.stage]}</div><h1>${lines(p.title[lang])}</h1><p class="detail-summary">${escape(p.summary[lang])}</p><dl class="project-facts"><div><dt>${t.role}</dt><dd>${escape(p.role[lang])}</dd></div><div><dt>${t.period}</dt><dd>${escape(p.period[lang])}</dd></div><div><dt>${t.stack}</dt><dd>${p.tags.map(escape).join(' · ')}</dd></div></dl></header><figure class="detail-cover"><img src="../../../${p.cover || `assets/${p.diagram}-${lang}.svg`}" alt="${escape(p.coverAlt?.[lang] || p.title[lang].replaceAll('\n',' '))}" width="960" height="600">${p.cover ? (p.coverCaption?.[lang] ? `<figcaption>${escape(p.coverCaption[lang])}</figcaption>` : '') : `<figcaption>${t.concept}</figcaption>`}</figure>${highlights}<div class="article-layout"><aside class="article-nav"><p>${t.toc}</p><nav aria-label="${t.toc}">${headings.map(h => `<a href="#${h.id}">${escape(h.title)}</a>`).join('')}</nav></aside><div class="article-content"><article class="prose">${article}</article>${media}${resources}<p class="evidence-note">${escape(p.evidenceNote?.[lang] || t.note)}</p></div></div><a class="next-project" href="../${next.slug}/"><span>${t.next}</span><strong>${escape(next.title[lang].replaceAll('\n',' '))}</strong><span aria-hidden="true">↗</span></a></main>`;
