...
export async function build() {
  ...
  for (const lang of ['ko', 'en']) {
    ...
    for (let i = 0; i < projects.length; i++) {
      ...
      const detail = `<main id="main" class="detail-main"><a class="back-link" href="../../#work">← ${t.back}</a><header class="detail-header"><div class="eyebrow">${t[p.category]} / ${t[p.stage]}</div><h1>${lines(p.title[lang])}</h1><p class="detail-summary">${escape(p.summary[lang])}</p><dl class="project-facts"><div><dt>${t.role}</dt><dd>${escape(p.role[lang])}</dd></div><div><dt>${t.period}</dt><dd>${escape(p.period[lang])}</dd></div><div><dt>${t.stack}</dt><dd>${p.tags.map(escape).join(' · ')}</dd></div></dl></header><figure class="detail-cover"><img src="../../../${p.cover || `assets/${p.diagram}-${lang}.svg`}" alt="${escape(p.title[lang].replaceAll('\n',' '))}" width="960" height="600">${p.cover ? '' : `<figcaption>${t.concept}</figcaption>`}</figure><div class="article-layout"><aside class="article-nav"><p>${t.toc}</p><nav aria-label="${t.toc}">${headings.map(h => `<a href="#${h.id}">${escape(h.title)}</a>`).join('')}</nav></aside><div class="article-content"><article class="prose">${article}</article>${media}${resources}<p class="evidence-note">${t.note}</p></div></div><a class="next-project" href="../${next.slug}/"><span>${t.next}</span><strong>${escape(next.title[lang].replaceAll('\n',' '))}</strong><span aria-hidden="true">↗</span></a></main>`;
